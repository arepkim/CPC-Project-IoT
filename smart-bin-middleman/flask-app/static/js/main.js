...
async function fetchData() {
    try {
        ...
        // Update Environment Alert Badge
        const envBadge = document.getElementById('env-alert');
        if (envBadge) {
            envBadge.innerText = data.env_alert || "Good";
            if (data.env_alert && data.env_alert.includes('Good')) {
                envBadge.className = 'badge bg-success';
            } else if (data.env_alert && data.env_alert.includes('WARNING')) {
                envBadge.className = 'badge bg-warning text-dark';
            } else if (data.env_alert && data.env_alert.includes('ALERT')) {
